...
CAPI_CLUSTER_NAME="${RUNNER_NAME}-${CLUSTER_NAME}-${INFRA}"
...
do
  ...
  clusterctl generate cluster "${CAPI_CLUSTER_NAME}${i}" \
    ...
    --infrastructure k3s | sed "s/10.243.0.0\/16/$POD_CIDR_ESC/g" | ${KUBECTL} apply -f -
done
...
do
  ...
  "${HELM}" repo update
  "${HELM}" install kyverno kyverno/kyverno -n kyverno --create-namespace --kubeconfig "${TMPDIR}/kubeconfigs/liqo_kubeconf_${i}"
done
