...
| fabric.imageName | string | `"ghcr.io/liqotech/fabric"` | Image repository for the route pod. |
| fabric.pod.annotations | object | `{}` | Annotations for the route pod. |
| fabric.pod.extraArgs | list | `[]` | Extra arguments for the route pod. |
| fabric.pod.labels | object | `{}` | Labels for the route pod. |
| fabric.pod.resources | object | `{"limits":{},"requests":{}}` | Resource requests and limits (https://kubernetes.io/docs/user-guide/compute-resources/) for the route pod. |
| fabric.tolerations | list | `[]` | Extra tolerations for the route daemonset. |
